        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form id="submitForm" action="/manage/notices" class="form-horizontal form-row-seperated" method="POST" enctype='multipart/form-data'>
                        {!! csrf_field() !!}
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                <select class="bs-select form-control" name="FNOTICEEXCELTYPE">
                                    <option value="10">每日通告单</option>
                                    <option value="20">预备通告单</option>
                                </select>
                            </div>
                        </div>
                        <?php $noticeGroups = ['A', 'B', 'C', 'D', 'E']; ?>

                        @foreach($noticeGroups as $key=>$noticeGroup)
                            <div class="form-group">
                                <label class="col-md-4">
                                    {{$key+1}}. <input class="form-control" type="text" name="groupName[]" maxlength="15" placeholder="例:文戏组或张导组等">
                                    <span class="help-block text-danger">如不填写则默认为"{{ $noticeGroup }}组"</span>
                                    <input type="hidden" name="upload_notice_number" value="{{ $key+1 }}">
                                    <input type="hidden" id="upload_notice_file_name{{ $key+1 }}" name="upload_notice_file_name[]">
                                    <input id='upload_notice_file_url{{$key+1}}' type="hidden" name="upload_notice_file_url[]" value="">
                                </label>
                                <div class="col-md-8">
            ...
            $("#dropz" + i).dropzone({
                ...
                init: function () {
                    ...
                    this.on('success', function (file, response) {
                        $("#upload_notice_file_url" + this.number).val(response.data.upload_file_url);
                        $("#upload_notice_file_name" + this.number).val(response.data.upload_file_name);
