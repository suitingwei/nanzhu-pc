                        ...
                        @foreach($un_receivers as $receiver)
                            <tr>
                                <td>
                                    ...
                                    <?php $firstJoinGroup = $unReceiveUser->groupsInMovie($movie_id)->first(); ?>
                                    {{--@if($firstJoinGroup){{ $firstJoinGroup->FNAME}}  @endif--}}
                                    {{ $unReceiveUser->groupNamesInMovie($movie_id) }}
                                </td>
                                <td>
                                    {{$receiver['job']}}
                                </td>
                                <td>
                                    ...
                                    <div class="dropdown">
										<a id="dLabel" @if($unReceiveUser->sharePhonesInMovie($movie_id)) href="javascript:;" @endif data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
											{{$receiver['username']}}
										</a>
										<ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
											@foreach($unReceiveUser->sharePhonesInMovie($movie_id)as $phone)
												@if($phone->FChecked)
													<li>{{$phone->FPHONE}}</li>
                        ...
                        @foreach($receivers as $receiver)
                            <tr>
                                <td>
                                    ...
                                    <?php $firstJoinGroup = $user->groupsInMovie($movie_id)->first(); ?>
                                    {{--@if($firstJoinGroup){{ $firstJoinGroup->FNAME}}  @endif--}}
                                    {{ $user->groupNamesInMovie($movie_id) }}
                                </td>
                                <td>
                                    {{$receiver['job']}}
                                </td>
                                <td>
                                    <?php $phones = \DB::table("t_biz_sparephone")->where("FGROUPUSERID", $receiver['group_user_id'])->get();?>
                                    <div class="dropdown">
                                        @if($user->isSharePhonesInMovieOpened($movie_id))
                                            <a id="dLabel" href="javascript:;" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                {{$receiver['username']}}
                                            </a>
                                            <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
                                                @foreach($user->sharePhonesInMovie($movie_id)as $phone)
                                                    @if($phone->FChecked)
                                                        <li>{{$phone->FPHONE}}</li>
                                                        <li role="presentation" class="divider"></li>
                                                    @endif
